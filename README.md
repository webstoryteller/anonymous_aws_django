...
(아래 오류 에러 해결 과정도 꼭 읽고 해당하는 내용 있으면 고칠 것. 로컬 pc에서 아무 문제 없이 잘 돌아가도 막상 배포하려면 알 수도 없는 문제가 생기는데 버전, 패키지 업데이트 등 생각지도 못한 곳에서 발생하는 경우가 대부분임. 아무래도 도커 (docker) 쓰는 것까지 배워야 배포를 자신 있게 할 수 있을 것 같음)
