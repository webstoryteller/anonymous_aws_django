...
def post_write(request):
    ...
    if request.method=="POST":
        ...
        if img:
            ...
            img_name = f"{datetime.now().strftime('%Y%m%d%H%M%S')}.{img_name}"
            # 어쩌.구.저쩌구.png
            ...
            # 장고 django 5.2.5 버전에서 default_storage의 3 저장 경로 URL을 터미널창에서 확인하려면 아래 세 줄 주석을 제거하고 실행한다.
            # 저장된 파일의 URL 가져오기. 웹페이지에서 등록 저장을 클릭하면 print 실행.
            ...
            default_storage.save(img_url, img) #AWS에 저장하는 것. 경로와 파일 이름. 위쪽 세 줄 주석을 제거하면 이 줄은 주석 처리해서 위 라인 명령어에 따라 터미널창에 나온다.
